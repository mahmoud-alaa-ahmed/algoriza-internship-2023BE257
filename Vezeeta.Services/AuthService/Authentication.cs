...
namespace Vezeeta.Services.AuthService
{
    public class AuthenticationService : IAuth
	{
		...
		public async Task<AuthModel> OnRegisterAsync(RegisterDTO regInfo)
		{
			...
			var imageUrl = regInfo.UserType.ToLower() == UserRole.Patient.ToLower() ? UploadHelpers.PatientImagePath : UploadHelpers.DoctorImagePath;

			 var uploadedFile = UploadHelpers.UploadFile(regInfo.Image!, imageUrl);
			...
			if(regInfo.UserType == UserRole.Patient)
